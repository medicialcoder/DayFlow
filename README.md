<div align="center">

# DayFlow

**每日任务记录及日报自动生成助手，本地浏览器可以直接打开使用！**

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![No Dependencies](https://img.shields.io/badge/Dependencies-Zero-orange)](DayFlow.html)
[![Single File](https://img.shields.io/badge/App-Single%20File-purple)](DayFlow.html)

一个 HTML 文件，下载下来双击就能用。<br>
没有 npm，没有构建，没有服务器，数据只存在你自己的设备上。
办公牛马必备，数据保存在本地可以随时回看。

[在线体验 →](https://medicialcoder.github.io/DayFlow/DayFlow.html)

</div>

---

## 截图

![DayFlow 演示](preview.gif)

---

## ✨ 功能

**📋 今日任务**
- 添加任务，设置优先级（高 / 中 / 低）、分类、预计时长、备注
- 完成、编辑、删除；支持拖拽手动调整顺序
- 可将任务安排到未来某天
- 按状态（待完成 / 已完成）和分类过滤

**📌 长期任务**
- 设置起止日期，手动更新进度（0–100%）
- 首页直接展示今天在周期内的长期任务及剩余天数

**📝 明日计划**
- 主页底部直接撰写明日工作安排，输入即自动保存
- 按 Enter 自动编号（1. / 2. / 3. …），写起来不打断思路
- 当天导出的日报会自动带上「明日计划」章节

**📅 历史记录**
- 查看任意历史日期的任务列表与当日明日计划
- 一键导出当天结构化工作日报（`.txt`），支持「重新生成」按当前任务状态刷新

**📊 统计与周报**
- 本周 / 本月 / 全部 三档统计：完成率、分类分布、优先级分布
- 一键生成本周 / 上周 / 上上周 / 三周前的工作周报，支持复制或导出

**📋 其他**
- 任务编号化输出，日报内容更整齐
- 侧栏完成率进度环，实时更新
- 近期 14 天有任务的日期快速入口
- 一键生成工作日报，支持复制或下载

---

## 🚀 快速开始

1. 下载本仓库的 `DayFlow.html`
2. 双击打开（默认会用系统浏览器加载）
3. 直接开始记录任务，所有数据都存在当前浏览器里

不需要安装任何东西，不需要联网。

---

## 📁 文件结构

```
DayFlow.html      全部逻辑（HTML + CSS + JS，单文件）
```

只需要这一个文件即可运行。

---

## 🔧 技术栈

纯原生，无任何第三方依赖：

| 技术 | 用途 |
|------|------|
| HTML / CSS / JavaScript | 全部应用逻辑 |
| localStorage | 本地数据持久化 |
| HTML5 Drag and Drop API | 任务拖拽排序 |

---

## 🔒 数据与隐私

所有数据仅保存在本地浏览器的 `localStorage` 中，**不经过任何服务器**。  
清除浏览器数据会丢失记录，建议定期使用「生成日报 → 导出」手动备份。

---

## License

[MIT](LICENSE)
